...
// import '../view_model/school_view_model/exam_time_table_view_model.dart';
...
//     });
//
//     final subVm = Provider.of<AllSubjectsVieModel>(context, listen: false);
//     await subVm.allSubjectsApi(context);
//     setState(() {
//       _subjects.addAll((subVm.allSubjectsModel?.data ?? []).map((e) => {
//         'id': e.subjectId.toString(),
//         'name': e.subjectName ?? '',
//       }));
//     });
//   }
...
//
//     final data = vm.allStudentListModel?.data ?? [];
//     final filtered = data.where((e) => e.sectionId.toString() == _selectedSectionId).toList();
//
//     setState(() {
...
//     });
//
//     final vm = Provider.of<CreateExamMarksViewModel>(context, listen: false);
...
//       final ok = await vm.createExamMarksApi(
//         int.tryParse(_selectedTimetableId ?? '0') ?? 0,
//         s.studentId ?? 0,
//         marks,
//         absent ? 1 : 0,
//         remarks,
...
//     setState(() {
//       _saving = false;
//       if (success > 0) _successMsg = '$success marks saved successfully!';
//     });
//   }
//
//
...
//   //     final s = row['student'];
//   //     final bool absent = row['isAbsent'];
//   //     final String marksText =
//   //     (row['marksCtrl'] as TextEditingController).text.trim();
//   //
//   //     // ── Grade decide karo marks ke basis pe ──
//   //     final double marksVal = absent ? 0.0 : (double.tryParse(marksText) ?? 0.0);
//   //     final String selGrade = _getGrade(marksVal, absent);
//   //
//   //     // ── existing check: agar student ka pehle se marksheet hai to UPDATE, warna CREATE ──
//   //     // NOTE: Agar aapke paas existing marksheet id available ho row mein, use karo
//   //     // Abhi hum assume kar rahe hain saare naye hain (CREATE flow)
//   //     // Agar update chahiye to: row['existingId'] != null check karo
//   //
//   //     final dynamic existingId = row['existingId']; // null = new, int = update
//   //
//   //     bool ok = false;
//   //
//   //     if (existingId == null) {
//   //       // ✅ CREATE
//   //       final response =
//   //       await Provider.of<CreateAdminMarkSheetViewModel>(
//   //         context,
//   //         listen: false,
//   //       ).createAdminMarkSheetApi(
//   //         s.studentId ?? 0,         // student id
//   //         int.tryParse(_selectedSubjectId ?? '0') ?? 0, // subject id
//   //         'term1',    // term (exam name/id as string)
//   //         selGrade,                 // grade (A, B, C, F etc.)
//   //          '2026-27',      // year — apna year variable lagao
//   //         context,
//   //       );
//   //       ok = response == true;
//   //     } else {
//   //       // ✅ UPDATE
//   //       ok = await Provider.of<UpdateSchoolAdminMarkSheetViewModel>(
//   //         context,
//   //         listen: false,
//   //       ).updateSchoolAdminMarkSheetApi(
//   //         existingId,   // grade_id (marksheet record id)
//   //         selGrade,     // updated grade
//   //         context,
//   //       );
//   //     }
//   //
...
//   //     _saving = false;
//   //     if (success > 0) {
//   //       _successMsg = '$success marks saved successfully!';
//   //     }
//   //     if (fail > 0) {
//   //       _snack('$fail records failed to save', Colors.red.shade400);
//   //     }
//   //   });
//   // }
//
// // ─────────────────────────────────────────────────────────────
// // 3️⃣  Helper — marks se grade calculate karo
// //     (apne school ke grading system ke hisaab se change karo)
// // ─────────────────────────────────────────────────────────────
//   String _getGrade(double marks, bool absent) {
//     if (absent) return 'F';
...
//
//                   // Stats Row (shown when students loaded)
...
//                           final progress = (((_cardAnim.value - delay) / (1.0 - delay)).clamp(0.0, 1.0));
//                           return Opacity(
//                             opacity: progress,
...
//           _statChip('Avg', '${_avg.toStringAsFixed(1)}%', AppColor.lightBlueColor,
...
//       child: Column(
//         children: [
//           // ── Top row: Avatar + Name + Absent toggle ──
//           Padding(
...
//           ),
//
//           // ── Progress bar (when marks entered) ──
//           if (hasMark)
//             Padding(
...
//           colors: [
//             AppColor.lightBlueColor,
//             AppColor.lightBlueColor.withBlue(255),
//           ],
//         ),
//         borderRadius: BorderRadius.circular(16),
//         boxShadow: [
//           BoxShadow(
//             color: AppColor.lightBlueColor.withOpacity(0.35),
//             blurRadius: 14,
...
//             const SizedBox(width: 8),
//             Text(
//               'Save All Marks  ($_total students)',
//               style: const TextStyle(
//                 color: Colors.white,
//                 fontWeight: FontWeight.w700,
//                 fontSize: 15,
...
//             Text(
//               'Select filters above & tap Load Students',
//               style: TextStyle(
//                 fontSize: 13,
//                 color: Colors.grey.shade400,
//               ),
...
class _CreateExamMarksPageState extends State<CreateExamMarksPage>
  ...
  // ── Timetable subjects (fetched after class+section selected) ──
  // Each entry: {timetable_id, subject_id, subject_name, ...}
  final List<Map<String, dynamic>> _timetableSlots = [];

  String? _selectedClassId;
  String? _selectedSectionId;
  String? _selectedExamId;
  String? _selectedSubjectId;   // subject_id from timetable
  String? _selectedTimetableId; // timetable_id — auto-set when subject chosen
  ...
  // ── Fetch timetable → populate subject dropdown ──
  // Called after both exam + class + section are selected
