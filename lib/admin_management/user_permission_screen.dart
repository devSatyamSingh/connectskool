...
class _UserPermissionsScreenState extends State<UserPermissionsScreen> {
  ...
  final List<Map<String, dynamic>> _roles = [
    {'key': 'school_admin', 'label': 'School Admin', 'icon': Icons.admin_panel_settings_rounded},
    {'key': 'teacher',      'label': 'Teacher',      'icon': Icons.person_rounded},
    {'key': 'student',      'label': 'Student',      'icon': Icons.school_rounded},
    {'key': 'accountant',   'label': 'Accountant',   'icon': Icons.account_balance_rounded},
  ];
  ...
  void initState() {
    ...
    final usersVm =
    Provider.of<GetUsersByRoleViewModel>(context, listen: false);

    // Agar already data loaded hai to dobara API mat call karo
    if (usersVm.users.isEmpty) {
      Future.microtask(() => _fetchUsersByRole(_selectedRole));
    }
  }
  // ─── Step 1: Role dropdown changed → fetch users ──────────────────────

  void _fetchUsersByRole(String role) async {
    setState(() => _selectedRole = role);

    final usersVm = Provider.of<GetUsersByRoleViewModel>(context, listen: false);
  ...
  void _fetchPermissions(GetUsersByRoleViewModel usersVm) {
    final userId = int.tryParse(
      usersVm.selectedUser?.userId?.toString() ?? '0',
    ) ?? 0;

    Provider.of<GetUserPermissionViewModel>(context, listen: false)
  ...
  void _resetAll() =>
      Provider.of<GetUserPermissionViewModel>(context, listen: false).resetAll();

  // ─── Save ─────────────────────────────────────────────────────────────

  // void _saveChanges() {
  //   final vm = Provider.of<GetUserPermissionViewModel>(context, listen: false);
  //   debugPrint('Allowed IDs: ${vm.allowedIds}');
  //   debugPrint('Denied IDs:  ${vm.deniedIds}');
  //
  //   // TODO: call AssignRoleViewModel with vm.allowedIds / vm.deniedIds
  //
  ...
  //         side: const BorderSide(color: _greenAllow),
  //       ),
  //       margin: const EdgeInsets.all(16),
  //       content: const Row(children: [
  //         Icon(Icons.check_circle_rounded, color: _greenAllow, size: 18),
  //         SizedBox(width: 8),
  //         Text('Permissions saved successfully!',
  //             style: TextStyle(color: _greenAllow, fontWeight: FontWeight.w600, fontSize: 13)),
  //       ]),
  ...
  void _saveChanges() async {
    ...
    final saveVm =
    Provider.of<SaveUserPermissionViewModel>(context, listen: false);
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Failed to save permissions"),
        ),
      );
    }
  }  // ─── Build ────────────────────────────────────────────────────────────
  ...
  Widget _buildHeader() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: _accent.withOpacity(0.3), blurRadius: 14, offset: const Offset(0, 5))
        ],
      ),
      child: Row(
        children: [
          InkWell(
            ...
            child: const Padding(
              ...
              child: Icon(Icons.arrow_back_ios_new_rounded, color: _white, size: 20),
            ),
          ),
            ...
            child: Column(
              ...
              children: [
                Text('User Permissions',
                    style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: _white)),
                SizedBox(height: 2),
                Text('Settings  ›  User Permissions',
  ...
  Widget _buildLabel(String text) {
    return Text(text,
        style: const TextStyle(
            ...
            color: _textMid, letterSpacing: 0.6));
  }
    ...
    return _dropdownContainer(
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          onChanged: (val) { if (val != null) _fetchUsersByRole(val); },
          items: _roles.map((r) {
                ...
                child: Padding(
                  ...
                  child: Row(children: [
                    Icon(r['icon'] as IconData,
                        size: 16, color: isSelected ? _white : _textMid),
                    const SizedBox(width: 10),
                    Text(r['label'] as String,
                        style: TextStyle(
      ...
      children: [
        _dropdownContainer(
          child: DropdownButtonHideUnderline(
            child: DropdownButton<UserByRole>(
              ...
              onChanged: (user) { if (user != null) _onUserChanged(user); },
        ...
        if (usersVm.selectedUser?.userEmail != null) ...[
          ...
          Padding(
            ...
            child: Text(usersVm.selectedUser!.userEmail.toString(),
  ...
  Widget _dropdownContainer({required Widget child}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: _shadowColor.withOpacity(0.18), blurRadius: 6, offset: const Offset(0, 2))
        ],
      ),
      child: child,
    );
  ...
  Widget _buildBody(GetUsersByRoleViewModel usersVm, GetUserPermissionViewModel permVm) {
      ...
      child: Column(
        ...
        children: [
          const Text('Permissions Summary',
              style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.bold, color: _textDark)),
          const SizedBox(height: 14),
    ...
    return Column(
      children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Expanded(child: Text(label, style: const TextStyle(fontSize: 12.5, color: _textMid))),
          Text('$count', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: _textDark)),
        ]),
                ...
                Expanded(
                  child: Row(children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                          color: _screenBg, borderRadius: BorderRadius.circular(10)),
                ...
                GestureDetector(
                  ...
                  child: Row(children: [
                    ...
                    const Text('Select All',
                        style: TextStyle(fontSize: 11.5, color: _textMid, fontWeight: FontWeight.w500)),
                  ]),
                ),
              ],
            ),
  ...
  Widget _buildPermRow(GetUserPermissionViewModel vm, PermissionItem item) {
    ...
    // dot color
    Color dotColor;
    if (currentState == 'allowed')      dotColor = _greenAllow;
    else if (currentState == 'denied')  dotColor = _redDeny;
    else                                dotColor = _divider;
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              Row(children: [
                _permBtn('Allowed', currentState == 'allowed', _greenAllow, _greenBg,
                        () => vm.updatePermissionState(permId, currentState == 'allowed' ? 'default' : 'allowed')),
                const SizedBox(width: 5),
                _permBtn('Denied', currentState == 'denied', _redDeny, _redBg,
                        () => vm.updatePermissionState(permId, currentState == 'denied' ? 'default' : 'denied')),
                const SizedBox(width: 5),
                _permBtn('Default', currentState == 'default', _accent, _accentLight,
  ...
  Widget _permBtn(String label, bool active, Color activeColor, Color activeBg, VoidCallback onTap) {
      ...
      child: Row(children: [
        Expanded(
          child: OutlinedButton.icon(
            ...
            icon: const Icon(Icons.refresh_rounded, size: 15, color: _textMid),
            label: const Text('Reset to Default',
                style: TextStyle(color: _textMid, fontWeight: FontWeight.w600, fontSize: 12.5)),
          ),
        ),
        const SizedBox(width: 12),
        Expanded(
          ...
          child: DecoratedBox(
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(10),
              boxShadow: [
                BoxShadow(color: _accent.withOpacity(0.3), blurRadius: 10, offset: const Offset(0, 3))
              ],
            ),
            child: ElevatedButton.icon(
              ...
              label: const Text('Save Changes',
                  style: TextStyle(color: _white, fontWeight: FontWeight.w700, fontSize: 12.5)),
            ),
          ),
        ),
      ]),
  ...
  Widget _card({required Widget child, EdgeInsetsGeometry? padding}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: _shadowColor.withValues(alpha: 0.2), blurRadius: 8, offset: const Offset(0, 3))
        ],
      ),
      child: child,
    );
