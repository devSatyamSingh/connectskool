...
class _AllStudentListState extends State<AllStudentList>
  ...
  void _openAddForm() {
    if (!PermissionGuard.check(
        context, PermissionKeys.addStudent, "Add Student")) {
      return;
    }
    Navigator.push(
      context,
  ...
  // ── Navigate to Edit form (pre-filled) ───────────────────
  void _openEditForm(Map<String, dynamic> student) {
    if (!PermissionGuard.check(
        context, PermissionKeys.editStudent, "Edit Student")) {
      return;
    }
    Navigator.push(
      context,
      MaterialPageRoute(
          builder: (_) => StudentFormPage.edit(student: student)),
    );
  }
  ...
  // ── Build a normalised student map ────────────────────────
  ...
  Widget _animatedCard(int index, Map<String, dynamic> data) =>
      AnimatedBuilder(
        animation: _animCtrl,
        builder: (_, child) {
          ...
          builder: (_, val, __) => _filterDropdown(
            ...
            onChanged: (v) async {
              ...
              if (v != null) {
                ...
                final resp =
                await AllSectionsRepository().allSectionsApi(v);
    ...
    child: Row(children: [
      SizedBox(
        ...
        child: CircularProgressIndicator(
            strokeWidth: 2, color: AppColor.lightBlueColor),
      ),
